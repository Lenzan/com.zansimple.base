...
namespace Framework
{
	...
	public class OnceWaitTimerAwakeSystem : AwakeSystem<OnceWaitTimer, ETaskCompletionSource<bool>>
	{
		public override void Awake(OnceWaitTimer self, ETaskCompletionSource<bool> callback)
		{
	...
	{
		public ETaskCompletionSource<bool> Callback { get; set; }
		
		public void Run(bool isTimeout)
		{
			ETaskCompletionSource<bool> tcs = this.Callback;
	...
	public class TimerComponent : Entity
	{
		...
		public async ETask<bool> WaitTillAsync(long tillTime, ECancellationToken cancellationToken)
		{
			if (TimeHelper.Now() > tillTime)
			{
				return true;
			}
			ETaskCompletionSource<bool> tcs = new ETaskCompletionSource<bool>();
			OnceWaitTimer timer = EntityFactory.CreateWithParent<OnceWaitTimer, ETaskCompletionSource<bool>>(this, tcs);
		...
		public async ETask<bool> WaitTillAsync(long tillTime)
		{
			if (TimeHelper.Now() > tillTime)
			{
				return true;
		...
		public async ETask<bool> WaitAsync(long time, ECancellationToken cancellationToken)
		{
		...
		public async ETask<bool> WaitAsync(long time)
		{
			long tillTime = TimeHelper.Now() + time;
			ETaskCompletionSource<bool> tcs = new ETaskCompletionSource<bool>();
			OnceWaitTimer timer = EntityFactory.CreateWithParent<OnceWaitTimer, ETaskCompletionSource<bool>>(this, tcs);
